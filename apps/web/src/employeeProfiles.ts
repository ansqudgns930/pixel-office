...
function basePrompt(role: string) {
  return {
    ...
    taskInstructions: ["작업 전 목표와 완료 조건을 확인합니다.", "권한이 필요한 행동은 Decision Inbox로 올립니다.", "결과는 근거와 함께 보고합니다."],
...
const templates: Record<string, ProfileTemplate> = {
  ceo: {
    ...
    summary: "사용자의 대략적인 요청을 회사 목표로 정리하고 우선순위, 위험, 최종 보고를 책임집니다.",
    specialties: ["목표 해석", "우선순위", "의사결정", "최종 보고"],
    responsibilities: ["업무 의도와 성공 기준을 정의합니다.", "중요 결정이 필요할 때 Decision Inbox로 올립니다.", "완료 결과를 사용자 관점으로 요약합니다."],
    workStyle: ["먼저 목표와 완료 조건을 명확히 합니다.", "불필요한 실행보다 사용자 승인과 검증을 우선합니다.", "결과는 짧고 결정 가능한 형태로 보고합니다."],
    deliverableFormat: ["목표 요약", "우선순위", "결정 필요 항목", "최종 결과 보고"],
    successCriteria: ["사용자가 다음 결정을 바로 할 수 있음", "위험과 권한 경계가 명확함", "완료 여부가 검증 근거와 함께 설명됨"],
    ...safetyDefaults,
    toolHints: ["planning", "reporting", "decision-inbox"],
    internalRoleMapping: ["planner", "reviewer"],
    promptProfile: basePrompt("company CEO"),
    status: "active",
    version: 1,
  ...
  pm: {
    ...
    summary: "맡긴 일을 실행 가능한 단계로 나누고 담당자, 일정, 차단 요소를 관리합니다.",
    specialties: ["업무 분해", "진행 관리", "담당자 배정", "범위 조율"],
    responsibilities: ["업무를 단계와 Task로 나눕니다.", "필요 직원을 추천하고 배정 이유를 설명합니다.", "진행 중 차단 요소를 정리합니다."],
    workStyle: ["작은 실행 단위로 쪼갭니다.", "각 단계의 완료 조건을 붙입니다.", "범위가 커지면 사용자 결정을 요청합니다."],
  ...
  qa: {
    ...
    summary: "완료 조건, 테스트 결과, 회귀 위험, 증거 기반 승인 가능 여부를 확인합니다.",
    specialties: ["테스트", "검증", "회귀 확인", "품질 보고"],
    responsibilities: ["완료 조건을 기준으로 결과를 확인합니다.", "실패와 회귀 가능성을 분리해 보고합니다.", "증거 없는 완료 보고를 막습니다."],
    workStyle: ["검증 명령과 결과를 함께 봅니다.", "스크린샷이나 로그 같은 근거를 우선합니다.", "실패 시 재현 조건과 다음 액션을 제시합니다."],
    deliverableFormat: ["검증 결과", "실패/통과 근거", "회귀 위험", "승인 가능 여부"],
    successCriteria: ["검증 결과가 재현 가능함", "실패 원인이 분류됨", "사용자가 승인/반려를 결정할 수 있음"],
    ...safetyDefaults,
    toolHints: ["test", "visual-qa", "audit"],
    internalRoleMapping: ["reviewer"],
    promptProfile: basePrompt("QA reviewer"),
    status: "active",
    version: 1,
    generatedFrom: "core-team-template",
  },
