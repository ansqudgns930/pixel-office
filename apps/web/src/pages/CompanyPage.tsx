      ...
      {snapshot && portfolio && totals && (
        <>
          ...
          <div className="stat-grid" style={{ marginTop: 12 }}>
            ...
            <div className={`stat-tile${totals.approvals?" warning":""}`}><div className="label">대기 승인</div><div className="value">{totals.approvals}</div></div>
          ...
          <div hidden={activeTab !== "overview"} className="measurement-guidance"><strong>다음 운영 확인</strong><span>{totals.approvals>0?`대기 승인 ${totals.approvals}건을 검토하세요.`:totals.blocked>0?`차단 태스크 ${totals.blocked}건의 원인을 확인하세요.`:"현재 긴급 조치가 없습니다. 연결 프로젝트의 다음 태스크를 확인하세요."}</span></div>
