...
async function main() {
  ...
  const checks = [
    { name: 'company-home', url: `/company?companyId=${companyId}`, required: ['업무 흐름', '운영·관리', '고급', '처음이라면 작은 업무 하나를 AI 회사에 맡겨보세요', '무슨 일을 AI 회사에 맡길까요?', 'AI 팀에게 계획 요청', '결정 필요'] },
    { name: 'staff-team', url: `/employees?companyId=${companyId}`, required: ['직원·AI팀', '고정 핵심팀', 'CEO', 'PM', 'Designer', 'Developer', 'QA', '외부 전문가 대기실'] },
    { name: 'employee-profile', url: `/employees?companyId=${companyId}&agentId=demo-designer`, required: ['직무기술서', '맡길 수 있는 일', '일하는 방식', '결정 필요', 'Prompt profile preview'] },
    ...
    { name: 'pixel-goal-focus', url: `/pixel-office?companyId=${companyId}&goalId=demo-company-goal`, required: ['선택 목표 추적 중', '지금 AI 팀이 하는 일', '현재 단계', '우선 신호', '목표 상세', '결정 필요', '결과·활동', '진행 업무'] },
  ...
  for (const item of checks) {
    ...
    if (item.name === 'employee-profile') {
      await page.getByRole('button', { name: '직무기술서' }).click();
